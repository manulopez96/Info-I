...
int main(void) {
	...
	printf("  -----------------Puntajes---");
	for(int j=0;j<10;j++){ 	//bucle para crear barra superior e identificar respuestas				
		printf("--%d",j+1); 			
	} 	
	...
	for(int i=0;i<5;i++){ 	
		...
		for(int j=0;j<promedio-0.5;j++){ 					
			printf("|||"); 	//llenado de grafico tabular		
		} 
		for(int j=0;j<9.5-promedio;j++){ 					
			printf("   "); 	//crea espacios  necesarios para alinear promedio en su columna		
		} 
